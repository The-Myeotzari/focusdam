...
const AccountDeleteDialogMenuItem = dynamic(() =>
  import('@/widgets/settings/ui/account-delete-dialog-menu-item').then(
    (module) => module.AccountDeleteDialogMenuItem
  )
);
...
const MENU_ITEM_CLASS_NAME =
  'group flex min-h-[88px] w-full items-center gap-4 rounded-[var(--radius-xl)] px-4 py-3 text-left transition-colors hover:bg-[var(--color-surface-container-low)] focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-2 focus-visible:outline-[color:var(--color-primary)]';

type MenuItemContentProps = Omit<Props, 'href' | 'action'>;

function MenuItemContent({ title, description, icon: Icon, tone }: MenuItemContentProps) {
  return (
    <>
      <span
...
export function SettingsMenuItem({ href, action, ...item }: Props) {
  if (action === 'delete-account-dialog') {
    return (
      <li>
        <AccountDeleteDialogMenuItem
          className={MENU_ITEM_CLASS_NAME}
          description={item.description}
          title={item.title}
          toneClassName={TONE_CLASS_NAMES[item.tone]}
        />
      </li>
    );
  }
  ...
  return (
    <li>
      <Link href={href} className={MENU_ITEM_CLASS_NAME}>
