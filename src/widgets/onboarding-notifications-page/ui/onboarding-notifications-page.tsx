...
export function OnboardingNotificationsPage() {
  ...
  const saveSettings = () => {
    updateOnboardingDraft(settings);
  };

  return (
    <main
      aria-labelledby="notifications-title"
      className="mx-auto flex min-h-[100svh] w-full max-w-[var(--page-max-width)] flex-col items-center bg-[#faf9fc] font-['42dot_Sans','Hanken_Grotesk','Noto_Sans_KR',sans-serif]"
    >
      <SiteTopBar
        title="집중이담"
      ...
      <section className="flex w-full flex-1 flex-col items-center px-5 pb-14 pt-10 sm:pt-12">
        ...
        <section className="mt-14 flex w-full flex-col gap-6" aria-label="추천 알림">
          {reminders.map((reminder) => (
            <ReminderCard
              key={reminder.title}
              {...reminder}
              enabled={settings[reminder.setting]}
              onToggle={() =>
                setSettings((current) => ({
                  ...current,
                  [reminder.setting]: !current[reminder.setting]
                }))
              }
            />
          ))}
