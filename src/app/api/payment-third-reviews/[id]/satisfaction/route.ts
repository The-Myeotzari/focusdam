...
import { CompletePaymentThirdReviewSatisfactionRequestSchema } from '@/entities/payment-third-review/api/payment-third-review-satisfaction.schema';
...
export async function PATCH(request: Request, { params }: RouteContext) {
  ...
  if (!result.ok) {
    if (result.reason === 'not_found' || result.reason === 'invalid_type') {
      return apiError(request, 'NOT_FOUND', 404, '만족도 체크 대상을 찾을 수 없습니다.');
    }

    if (result.reason === 'already_completed') {
      return apiError(request, 'ALREADY_COMPLETED', 409, '이미 만족도 체크를 완료했습니다.');
    }

    if (result.reason === 'not_ready') {
      return apiError(request, 'NOT_READY', 409, '아직 만족도 체크를 진행할 수 없습니다.');
    }
