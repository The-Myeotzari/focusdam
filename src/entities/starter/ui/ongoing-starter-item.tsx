...
export function OngoingStarterItem({ starter }: Props) {
  const description = formatStarterSchedule(starter.startedAt, starter.durationMinutes);

  return (
    // 빌드 오류가 발생하여 임의로 Link 주석 처리
    // <Link
    //   href={`/starters/${starter.id}`}
    <article className="flex min-h-[104px] items-center gap-4 rounded-[32px] bg-[var(--color-surface-container-low)] px-6 transition hover:bg-[var(--color-surface-container)]">
      ...
      <ChevronRight size={20} className="shrink-0 text-[var(--color-outline)]" aria-hidden="true" />
      {/* </Link> */}
    </article>
  );
}
